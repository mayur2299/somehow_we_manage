...
export default defineEventHandler(async (event) => {
  const body = await readBody<{ ward?: string; service?: string; note?: string; tag?: string; photo?: string; locality?: string }>(event)
  ...
  const ts = Date.now()
  const flag: Flag = { id: `${ts}-${Math.random().toString(36).slice(2, 8)}`, ward, service, note, tag, photo, ts, likes: 0, locality }
